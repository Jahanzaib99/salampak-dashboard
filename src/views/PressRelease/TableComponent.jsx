...
import {
    categoryPhotoImage, pressRelease
} from "config/routes";
...
class TableComponent extends Component {
    ...
    renderPagination = () => {
        ...
        if (this.state.pressRelease && this.state.pressRelease.length > 0) {
            ...
            <Grid
                ...
                <Grid item style={{ marginBottom: '15px' }}>
                    <Typography variant="caption" gutterBottom>
                        Showing {`${skip + 1} - ${(skip + this.state.pressRelease && this.state.pressRelease.length)} of ${totalCount} `}
                    </Typography>
    ...
    render() {
        const { categories, isLoading, formFields, pressRelease } = this.state;
            ...
            <div className={classes.tableRoot}>
                <Paper className={classes.paperRoot}>
                    <Grid container direction="row" spacing={2}>
                        ...
                        {!isLoading && userPermissions?.pressRelase?.post && (
                            <>
                        ...
                        <Table className={classes.table}>
                            <TableHead className={classes.tableHeader}>
                                <TableRow>
                                    ...
                                    {
                                        userPermissions?.pressRelase?.update || userPermissions?.pressRelase?.delete ?
                                            <TableCell className={classes.givePointer} >
                                                <TableSortLabel className={classes.tableSortLabel}>
                                                    Actions
                                </TableSortLabel>
                            ...
                            <TableBody className={classes.tableHeader}>
                                {pressRelease && pressRelease.length > 0 && pressRelease.map((row, index) => (
                                            ...
                                            userPermissions?.pressRelase?.update || userPermissions?.pressRelase?.delete  ?
                                                <TableCell>
                                                    <Form
                                                        title="Press Release"
                                                        edit="category"
                                                        edit={userPermissions?.pressRelase?.update ? "category" : ""}
                                                        data={row}
                                                        fields={formFields}
                                                        update={userPermissions?.pressRelase?.update ? this.handleUpdate : false}
                                                        delete={userPermissions?.pressRelase?.delete ? this.handleDelete : false}
                                                    />
                                                </TableCell> : ""}
                                    </TableRow>
                                ))}

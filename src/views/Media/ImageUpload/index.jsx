...
class ImageUploadComponent extends Component {
    ...
    getEventImages = (id) => {
        axios
            ...
            .then(response => {
                log(`GET ${this.props.apiUrl}/${id} ===> Image fetch Component`, "info", response.data.data);
                let res = response.data.data;
                let data = {};
                data.links = res.photos;
                this.setState(data, () => this.state.links);
            })
            .catch(error => {
    ...
    render() {
        ...
        const { openDialog, viewDialog, viewImage, images, links, isLoading, activeStep } = this.state;
        // console.log(links);
        return (
            <div>
                ...
                <form className={classes.form} style={{ padding: '0', marginLeft: '-40px' }}>
                    {openDialog && (
                    ...
                    {images && images.length > 0 ? (
                            ...
                            <Grid container spacing={2}>
                                <GridList cols={3}>
                                    {images.map((item, index) => (
                                        <GridListTile key={index} style={{ width: '140px', height: '140px' }}>
                                            <img
                                                alt={`Crop - ${index}`}
                                                style={{
                                                    width: '500px',
                        ...
                        <React.Fragment>
                                    ...
                                    <div key={index} >
                                        {Math.abs(activeStep - index) <= 2 && (
                                            <span>
                                                ...
                                                <div style={{ width: '145px', marginTop: '-40px', background: 'black', opacity: 0.9 }}>
                                                    <Tooltip title="Delete Image" aria-label="delete">
                                                        <IconButton onClick={e => this.deleteImage(item)} className={classes.icon}>
                                                            <DeleteIcon />
                                                        </IconButton>
                                                    </Tooltip>
                                                    <Tooltip title="Upload more Image" aria-label="image">
                            ...
                            <MobileStepper
                                ...
                                nextButton={
                                    <IconButton aria-label="delete" onClick={this.handleNext} disabled={activeStep === links.length - 1} className={classes.margin} size="small">
                                        <KeyboardArrowRight fontSize="inherit" />
                                    </IconButton>
                                }
                                backButton={
                                    <IconButton aria-label="delete" onClick={this.handleBack} disabled={activeStep === 0} className={classes.margin} size="small">

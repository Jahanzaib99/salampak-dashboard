...
import { categoryPhotoImage } from "config/routes";
...
class TableComponent extends Component {
  ...
  populateTable = () => {
    ...
    axios
      ...
      .then((resp1) => {
        ...
        axios.get(tagTypesUrl).then((resp2) => {
          ...
          let dynamicField = {
            ...
            type: 5,
            options: resp2.data.data.categoryTypes,
          };
      ...
      >
        <Grid item style={{ marginBottom: "15px" }}>
          <Typography variant="caption" gutterBottom>
            Showing{" "}
            {`${skip + 1} - ${skip +
              this.state.categories.length} of ${totalCount} `}
          </Typography>
          <ButtonGroup size="small" style={{ verticalAlign: "middle" }}>
            <IconButton
              style={{ margin: 5 }}
              onClick={this.onClickFirstBtn}
              disabled={firstBtn}
            >
              <FirstPageIcon />
            </IconButton>
    ...
    this.setState(
      {
        ...
        skip: (this.state.lastPage - 1) * this.state.pageSize,
      },
      () => {
  ...
  render() {
    const { categories, isLoading, formFields, _objectId } = this.state;
            ...
            {!isLoading && (
              <>
                <Grid item xs={6} sm={6} md={1}>
                  <Form
                    title="Category"
                    create={userPermissions?.categories?.post ? "category" : ""}
                    fields={formFields}
                    submit={this.handleSubmit}
                    apiUrl={categoryPhotoImage}
                    _objectId={_objectId}
                    isTab={true}
                    paginationImgHide={true}
                    emptyObjectId={() =>
                      this.setState({
                        _objectId: "",
                      })
              ...
              <TableHead className={classes.tableHeader}>
                <TableRow>
                  ...
                  <TableCell
                    className={
                      classes.givePointer
                    } /* onClick={e => this.onSort(e, 'companyName', '', 'vendor')}*/
                  >
                    <TableSortLabel
                      className={
                        classes.tableSortLabel
                  ...
                  {userPermissions?.categories?.update ? <TableCell className={classes.givePointer} >
              ...
              <TableBody className={classes.tableHeader}>
                {categories.map((row, index) => (
                  <TableRow key={index} hover>
                    ...
                    {userPermissions?.categories?.update ? <TableCell>
                      <Form
                        title="Category"
                        edit={userPermissions?.categories?.update ? "category" : ""}
                        data={row}
                        fields={formFields}
                        ...
                        isTab={true}
                        userType={userType}
                        delete={userPermissions?.categories?.delete ? this.handleDelete : false}
                        /> </TableCell> : ""}
